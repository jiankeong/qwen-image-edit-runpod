...
The model card specifies NVIDIA Blackwell RTX 50-series, CUDA 13.0+; this image uses CUDA 13.0 and PyTorch cu130. Select RTX 5090 (32 GB) or RTX PRO 6000 Blackwell (96 GB), not L4/4090/A100. Hub smoke tests request the latter; they use a mock and do not prove GPU inference. The model files total about 16 GB; give the Network Volume at `/runpod-volume` at least 35 GB free for downloads, cache and outputs. The bootstrap downloads directly to the volume and symlinks those files into ComfyUI model directories without duplicate copies. It starts ComfyUI locally and submits an API workflow using `UnetLoaderGGUF`, `CLIPLoaderGGUF`, and `TextEncodeQwenImageEditPlus`.
