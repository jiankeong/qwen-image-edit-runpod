...
The model card specifies NVIDIA Blackwell RTX 50-series, CUDA 13.0+; this image uses CUDA 13.0 and PyTorch cu130. Select RTX 5090 (32 GB) or RTX PRO 6000 Blackwell (96 GB), not L4/4090/A100. Hub smoke tests request the latter; they use a mock and do not prove GPU inference. The model files total about 16 GB; give the Network Volume at `/runpod-volume` at least 35 GB free for downloads, cache and outputs. The bootstrap reuses complete files already cached there. When the Network Volume lacks space for another component, it downloads that component into the container disk under `/tmp/qwen-hf-cache` instead, then symlinks it into ComfyUI. This temporary fallback downloads again on a cold worker, so expanding the Network Volume is preferable for persistent fast startup. Inspect capacity with `df -h /runpod-volume /tmp` and `du -sh /runpod-volume/hf-cache /runpod-volume/hf-home /runpod-volume/models 2>/dev/null`. The worker starts ComfyUI locally and submits an API workflow using `UnetLoaderGGUF`, `CLIPLoaderGGUF`, and `TextEncodeQwenImageEditPlus`.
